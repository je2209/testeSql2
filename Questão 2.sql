...
SELECT 
    ...
    COUNT(DISTINCT emp.matr) AS qtd_empregados,
    ROUND(AVG(sal.total_salario), 2) AS media_salarial,
    ROUND(MAX(sal.total_salario), 2) AS maior_salario,
    ROUND(MIN(sal.total_salario), 2) AS menor_salario
FROM departamento AS dept
JOIN empregado AS emp ON emp.lotacao = dept.cod_dep
JOIN (
    SELECT 
        empVenc.matr,
        COALESCE(SUM(venc.valor), 0) AS total_salario
    FROM emp_venc AS empVenc
    JOIN vencimento AS venc ON empVenc.cod_venc = venc.cod_venc
    GROUP BY empVenc.matr
